...
export function csvReducer(state = initialState, action = {}) {
    switch (action.type) {
        ...
        case "GET_INITIAL_SUCCESS":
            const formErrorsMappings = Object.keys(action.result).reduce(
                (obj, field) => {
                    obj[field] = null;
                    return obj;
                },
        ...
        case "SCHEDULE_INGEST_SUCCESS": {
            return {
                ...
                isScheduleError: false,
                status:
                    "Successfully uploaded csv file and scheduled ingest job. Look at left sidebar for status updates regarding the ingest job."
            };
        }
        ...
        case "FILE_UPLOAD_CANCELLED": {
            const destination = Object.keys(state.destination).reduce(
                (obj, dest) => {
                    obj[dest] = {
        ...
        case "TOGGLE_ADD_CUSTOM_FIELD_MODAL": {
            return {
                ...state,
                addCustomFieldModalActive:
                    action.result.action === "open" ? true : false
            };
        }
        ...
        case "CREATE_CUSTOM_FIELD_SUCCESS": {
            ...
            return {
                ...
                destinationArr: [
                    //immutablely adding item to array
                    //ref: https://redux.js.org/recipes/structuringreducers/immutableupdatepatterns
                    ...state.destinationArr.slice(
                        0,
                        state.destinationArr.length
                    ),
        ...
        case "DELETE_CUSTOM_FIELD_SUCCESS": {
            const formErrorsMappings = Object.keys(state.form.errors).reduce(
                (obj, field) => {
        ...
        case "RESET": {
            return {
                ...
                // status: null,
                ingestId: null,
                destination: Object.keys(state.destination).reduce(
                    (dest, key) => {
                        dest[key] = {
                ...
                form: {
                    ...
                    mappings: Object.keys(state.destination).reduce(
                        (dest, key) => {
                            dest[key] = {
                                ...state.destination[key],
                                value: null,
